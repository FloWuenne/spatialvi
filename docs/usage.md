...
| `fastq_dir`        | Path to directory where the sample FASTQ files are stored. May be a `.tar.gz` or `.tar` file instead of a directory.            |
...
| `spaceranger_dir` | Output directory generated by spaceranger. May be a `.tar.gz` or `.tar` file instead of a directory |
